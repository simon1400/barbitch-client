...
const BookServicePage = async () => {
  ...
  return (
    <Accordion type={'multiple'}>
      {data.map((group: IBookServiceGroup) => (
        <AccordionItem
          key={group.title}
        ...
        >
          ...
          <AccordionContent className={'px-3 pb-0'}>
            <ul>
              {group.group_event_types.map((service: IBookService) => (
                <li key={service.title} className={'border-t-2 border-[#3C3C3C] border-dotted'}>
                  <Link
                    className={
                      'flex py-3.5 px-1 items-center gap-4 hover:bg-[#3C3C3C] duration-200'
                    }
                    href={`/book/${service.id}`}
                  >
                    <span
                      className={
                        'min-w-[36px] w-[36px] h-[36px] overflow-hidden rounded-full block relative'
                      }
                    >
                      <Image
                    ...
                    <span className={'flex items-center text-xs1 text-primary font-bold gap-2.5'}>
                      ...
                      >{`${service.variations[0].prices[0].amount} Kč`}</span>
                      <span>
                        <ChevronRight />
                      </span>
                    </span>
