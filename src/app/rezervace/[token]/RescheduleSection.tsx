...
import type { IEngineDay, IEngineManageInfo } from '../../book/fetch/engine'
...
import { CalendarSkeletonInner } from '../../book/[serviceId]/[personalId]/components/CalendarSkeleton'
import {
  engineErrorCode,
  getEngineManageAvailability,
  postEngineReschedule,
} from '../../book/fetch/engine'

const BookDatePicker = dynamic(
  () => import('../../book/[serviceId]/[personalId]/components/DatePicker'),
  { ssr: false, loading: () => <CalendarSkeletonInner /> },
)
const TimePicker = dynamic(
  () => import('../../book/[serviceId]/[personalId]/components/TimePicker'),
  { ssr: false },
)
const EmptyAlert = dynamic(
  () => import('../../book/[serviceId]/[personalId]/components/EmptyAlert'),
  { ssr: false },
)
...
export const RescheduleSection = ({ token, onBack, onDone }: Props) => {
  ...
  const loadAvailability = useCallback(async () => {
    ...
    setLoadError('')
    try {
      const availability = await getEngineManageAvailability(
        token,
  ...
  const handleConfirm = async () => {
    ...
    setSubmitError('')
    try {
      const result = await postEngineReschedule(token, {
