      ...
      {data.map(({ title: categoryTitle, table }) => {
        ...
        return (
          <div key={categoryTitle} className={'pb-15'}>
            {table.map(({ sectionTitle, title: tableTitle, item }, tableIdx) => {
              const cols = getColumns(item)
              const hasCols = cols.length > 1 || (cols.length === 1 && tableTitle?.length)

              // Show section heading only for the first table in a consecutive group
              const prevSection = tableIdx > 0 ? table[tableIdx - 1].sectionTitle : undefined
              const showSection = !!sectionTitle && sectionTitle !== prevSection

              return (
                <React.Fragment key={`${sectionTitle ?? ''}-${tableTitle}`}>
                  {showSection && (
                    <h3 className={'text-sm11 font-bold md:text-sm1 uppercase mt-8 mb-4'}>
                      {sectionTitle}
                    </h3>
                  )}

                  <table className={'w-full max-w-full table-auto border-collapse mb-5'}>
                          ...
                          return (
                            ...
                            >
                              ...
                              {cols.includes('TOP MASTER') && (
                                <td className={`${TD_BASE} pl-1 md:pl-3.5`}>
                                  {topMasterPrice ?? ''}
                                </td>
                              )}
                            </tr>
